...
def create_event_date(time_ms, zone=None):
    ...
    zone_offset_sec = 0
    try:
        # If it's a datetime.timezone-like object, get offset
        if hasattr(zone, "utcoffset"):
            # Use current time for offset reference
            offset = zone.utcoffset(None)
            if offset is not None:
                zone_offset_sec = int(offset.total_seconds())
        elif isinstance(zone, int):
            # Direct integer offset (seconds)
            zone_offset_sec = zone
    except Exception:
        zone_offset_sec = 0
