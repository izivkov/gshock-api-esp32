...
# ---------------- Configuration ----------------
LOCAL_FOLDER = Path(".")   # project root
ESP_ROOT = Path("/")       # ESP32 root
...
# ---------------- Helper Functions ----------------
def list_local_files(directory: Path):
...
def list_esp_files():
    ...
    cmd = ["mpremote", "fs", "ls", "-r", str(ESP_ROOT)]
...
def delete_extra_files_dry_run(local_files, esp_files, dry_run=True):
    """Dry-run or delete ESP32 files not present locally."""
    local_set = {normalize(f) for f in local_files}
    for f in esp_files:
        f_norm = normalize(f)
        if f_norm.endswith("/"):
            continue
        if f_norm not in local_set:
            msg = f"[Dry-run] Would delete: {f_norm}" if dry_run else f"Deleting: {f_norm}"
            print(msg)
            if not dry_run:
                subprocess.run(["mpremote", "fs", "rm", f_norm])
...
def copy_file(local_file):
    ...
    remote_path = (ESP_ROOT / local_path).as_posix()
